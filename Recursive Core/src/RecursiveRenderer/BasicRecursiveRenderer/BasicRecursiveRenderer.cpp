...
Shapes<T> BasicRecursiveRenderer<T>::get_shapes_on_camera() const {
  ...
  // shapes.sort_by_size(); // this is too slow
...
template<typename T>
void BasicRecursiveRenderer<T>::process_zoom(Position<T> pointer, T scale_value) {
  update_data_structure_2d_with_new_shapes();
}

template<typename T>
void BasicRecursiveRenderer<T>::process_zoom_reset() {
...
template<typename T>
void BasicRecursiveRenderer<T>::process_confirm() {
  update_data_structure_2d_with_new_shapes();
}
